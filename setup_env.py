...
def detect_gpu() -> dict:
    ...
    if info["nvidia_gpu"] and VENV_PY.exists():
        r = _run([str(VENV_PY), "-c",
                 ...
                 timeout=60)
        device_count_ok = r.returncode == 0 and r.stdout.strip().isdigit() \
            and int(r.stdout.strip()) > 0
        info["cuda_runtime"] = device_count_ok
        if device_count_ok:
            # Device-count succeeding does not guarantee the actual runtime
            # libraries (cublas/cudnn) are installed — try a trivial GPU
            # allocation, which is what actually needs them.
            probe = _run([str(VENV_PY), "-c",
                         "import ctranslate2 as c; "
                         "c.StorageView(shape=[1], dtype='float32', device='cuda')"],
                         timeout=60)
            if probe.returncode != 0:
                info["cuda_runtime"] = False
                info["cuda_libs_missing"] = True
    if info["cuda_runtime"]:
        info["mode"] = f"GPU ({info['gpu_name']})"
        log(f"acceleration: GPU — {info['gpu_name']} (Whisper large-v3, NVENC "
            "if available)")
    elif info.get("cuda_libs_missing"):
        info["mode"] = "CPU (GPU present, CUDA runtime libraries missing)"
        log(f"NVIDIA GPU detected ({info['gpu_name']}) and the driver "
                 "works, but the CUDA runtime libraries (cublas/cudnn) "
                 "are not installed — running on CPU. The app will still "
                 "fall back automatically per-job if this happens later. "
                 "To enable GPU mode, install the CUDA 12.x runtime "
                 "(cudnn + cublas) matching your driver, e.g. via the "
                 "NVIDIA CUDA Toolkit installer, then run setup again.")
