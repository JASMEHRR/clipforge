...
- [!] Gotcha noted this session: this dev machine has live YouTube OAuth, so `POST /api/youtube/sync-schedule` runs against the real channel — keep manual/smoke checks read-only. It was harmless here only because the Part-4 approval gate returned 0 approved candidates.
