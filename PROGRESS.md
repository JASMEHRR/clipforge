...
Current: **feature/double-caption-fix** — subtitle_detect false-negative fixed + hard invariant check added (see Phase D below); the specific clip from the reported screenshot could not be conclusively identified in the diagnosis job and needs a human follow-up (see Known below). feature/ui-rework merged to main — UI rework + branding/fonts/provenance/updater verification complete (see Phase U below). Remaining manual items unchanged (Docker daemon build; optional YouTube OAuth).
