...
- [!] Known: deploy/huggingface still describes the old Gradio Space (needs its own rework or removal — human call). Per-run provider override and viral_v2.allow_upload have no UI control (Settings-level provider; privacy gate stays default-off). scripts/screenshot_ui.py and gate1.py deleted with app.py.
