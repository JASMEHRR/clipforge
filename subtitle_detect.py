...
def _detect_uncached(video_path: Path, start: float, end: float,
    ...
    try:
        ...
        # Full-frame height comes from the first successful grab.
        row_votes: np.ndarray | None = None
        frame_h = 0
        sampled = 0
        for i in range(n):
            ...
            if row_votes is None:
                frame_h = H
                row_votes = np.zeros(H, dtype=np.float64)
            y0 = int(H * (1.0 - search_band))
            for (rt, rb) in _text_lines_in_region(gray[y0:H, :]):
                row_votes[y0 + rt: y0 + rb] += 1.0
            sampled += 1
        if row_votes is None or sampled == 0:
            return {"present": False, "band_top_pct": 0.0, "band_bottom_pct": 0.0,
                    "confidence": 0.0, "sampled_frames": 0}
        persist = row_votes / sampled
        hot = persist >= persistence
        top, bottom, conf = _largest_run(hot, persist)
        if top is None:
            return {"present": False, "band_top_pct": 0.0, "band_bottom_pct": 0.0,
                    "confidence": 0.0, "sampled_frames": sampled}
        return {
            "present": True,
            "band_top_pct": round(top / frame_h, 4),
            "band_bottom_pct": round(bottom / frame_h, 4),
