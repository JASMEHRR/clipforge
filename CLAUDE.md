...
## Double-caption fix (feature/double-caption-fix)
- subtitle_detect.py: `_detect_uncached` now finds a candidate band via a `window_seconds` sliding window (config `style.subtitle_detect.window_seconds`, default 6.0), then only accepts it if the band is continuous for >= `persistence_ratio` of the whole clip OR recurs across >= 2 separate >=2s bursts. Fixes a false-negative where a genuinely-visible but intermittent burned caption (on/off per dialogue line) got diluted below `persistence_ratio` by the old whole-clip average. A naive "any window passes" version of this regressed on real footage (a static lit marquee sign held still for ~12s false-positived) — the continuous-or-recurring rule is load-bearing, don't simplify it back to "present in any window."
- `subtitle_detect.verify_no_leftover_subs(final_path, exclude_top_pct, exclude_bottom_pct, cfg)` — hard invariant, wired into `pipeline.py`'s `_render_clip` right after `caption_clip`/music. Runs when `edit_plan["existing_subs"]["decision"] in ("replace","keep")`; re-scans the FINAL rendered clip for a text-like band outside ClipForge's own caption/CTA zone (anchor ± margin). Warning lands on `metadata.json`'s `style.double_caption_warning`, never raises/kills the clip.
- See PROGRESS.md Phase D "Known" — the specific clip from the originally-reported screenshot was not confirmed in the available diagnosis job; don't assume clip_02 of `output/20260710-144449_url` was the real bug (it wasn't, on closer inspection — its original render was overwritten before verification, a process mistake noted there).
